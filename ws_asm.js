...
const keywords = require('./ws_core').keywords;

// Collect keywords
const mnemo = {};
keywords.forEach(keyword => mnemo[keyword.mnemo] = keyword);
      ...
      action: function (params, builder) {
        ...
        if (!(fileName in builder.includes)) {
          ...
          if (builder.includes[fileName]) {
            ...
            try {
              var ext = compile(builder.includes[fileName], builder);
...
function parseWhitespace(strArr) {
  var space = "";
...
function parseLineComment(strArr) {
  var comment = "";
  do {
...
function parseLabel(strArr, builder) {
  var label = "";
...
function getTokens(strArr, builder) {
  var tokens = [];
...
function pushInstruction(builder, constr, paramNumber) {
